...
class TestRandomSample:
    ...
    def test_returns_best(self, sphere_fitness, standard_bounds):
        """RandomSample returns the best value found across all samples."""
        opt = RandomSample(sphere_fitness, standard_bounds, seed=42)
        result = opt.run(100)
    ...
    def test_n_evaluations(self, sphere_fitness, small_bounds):
        """n_evaluations <= n_iterations (cache hits on small space)."""
        opt = RandomSample(sphere_fitness, small_bounds, seed=42)
        result = opt.run(50)
    ...
    def test_seeding(self, sphere_fitness, standard_bounds):
        ...
        opt1 = RandomSample(sphere_fitness, standard_bounds, seed=123)
        result1 = opt1.run(50)

        opt2 = RandomSample(sphere_fitness, standard_bounds, seed=123)
        result2 = opt2.run(50)
    ...
    def test_result_fields(self, sphere_fitness, standard_bounds):
