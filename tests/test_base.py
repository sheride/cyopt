...
class TestABC:
    def test_abc_not_instantiable(self, sphere_fitness, standard_bounds):
        """DiscreteOptimizer cannot be instantiated directly."""
        with pytest.raises(TypeError):
            DiscreteOptimizer(
                fitness_fn=sphere_fitness, bounds=standard_bounds
            )

    def test_concrete_subclass_instantiable(self, sphere_fitness, standard_bounds):
        """A concrete subclass implementing _step can be instantiated."""
        opt = _DummyOptimizer(fitness_fn=sphere_fitness, bounds=standard_bounds)
        assert opt is not None


class TestRun:
    def test_run_returns_result(self, sphere_fitness, standard_bounds):
        """run() returns a Result with correct fields."""
        opt = _DummyOptimizer(
            fitness_fn=sphere_fitness, bounds=standard_bounds, seed=42
        )
        result = opt.run(10)
    ...
    def test_history_length_equals_iterations(self, sphere_fitness, standard_bounds):
...
class TestCaching:
    def test_evaluate_caches_results(self, sphere_fitness, small_bounds):
        """Calling fitness twice with same DNA results in n_evaluations == 1."""
        opt = _DummyOptimizer(
            fitness_fn=sphere_fitness, bounds=small_bounds, seed=42
        )
        dna = (1, 1)
...
class TestBestTracking:
    def test_best_monotonically_non_increasing(self, sphere_fitness, standard_bounds):
...
class TestSeeding:
    def test_seeding_reproducibility(self, sphere_fitness, standard_bounds):
        ...
        opt1 = _DummyOptimizer(
            fitness_fn=sphere_fitness, bounds=standard_bounds, seed=123
        )
...
class TestProgress:
    def test_progress_no_raise(self, sphere_fitness, standard_bounds):
...
class TestHistory:
    def test_record_history_true(self, sphere_fitness, standard_bounds):
    ...
    def test_record_history_false(self, sphere_fitness, standard_bounds):
...
class TestRandomDNA:
    def test_random_dna_within_bounds(self, sphere_fitness, standard_bounds):
        """_random_dna() produces tuples within bounds."""
        opt = _DummyOptimizer(
            fitness_fn=sphere_fitness, bounds=standard_bounds, seed=42
        )
        for _ in range(100):
