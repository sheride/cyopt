...
class TestMCMC:
    ...
    def test_finds_improvement(self):
        ...
        opt = MCMC(sphere_fitness, BOUNDS_3D, seed=42)
        result = opt.run(100)
    ...
    def test_invalid_temperature_zero(self):
        ...
        with pytest.raises(ValueError, match="temperature must be positive"):
            MCMC(sphere_fitness, BOUNDS_3D, temperature=0)

    def test_invalid_temperature_negative(self):
        """Negative temperature raises ValueError."""
        with pytest.raises(ValueError, match="temperature must be positive"):
            MCMC(sphere_fitness, BOUNDS_3D, temperature=-1)

    def test_seeding(self):
        """Same seed produces identical results."""
        opt1 = MCMC(sphere_fitness, BOUNDS_3D, seed=777)
        result1 = opt1.run(50)

        opt2 = MCMC(sphere_fitness, BOUNDS_3D, seed=777)
        result2 = opt2.run(50)
    ...
    def test_record_history(self):
        ...
        opt = MCMC(sphere_fitness, BOUNDS_3D, seed=42, record_history=True)
        result = opt.run(20)
